...
2. 题目中所有问到的数字都必须正确，任何一个数字错误即判错
   （如题目问"小明赚了多少？店员亏了多少？"，则两个数字都必须对，
   "赚45亏95"应判错，因为店员亏的数字不对）
3. 答非所问或空回复判错
4. 方向一致且所有金额数字正确才判对
5. 回复可能包含详细的推理过程和自我纠正，只要最终有明确结论即视为完成，
   不要因中间过程长或有多处修正就判"未完成"或"未给出最终答案"。
   特别注意：即使回复很长（几百上千字），只要末尾有"结论"或"答案是"之类
   的明确总结，就是完整回复，不能判为"截断"或"未结束"。

请只回复一个 JSON 数组，包含所有 key_index ({keys_list}) 的判定：
[
  {{"key_index": <实际的key编号>, "correct": true, "reason": "答对"}},
  {{"key_index": <实际的key编号>, "correct": false, "reason": "数字错误，把50元当5元"}}
]
注意：key_index 必须与上方各 key 的编号一致，不要固定写 0。"""
